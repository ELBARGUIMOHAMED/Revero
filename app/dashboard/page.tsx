...
export default async function DashboardPage() {
  ...
  const stats = [
    { title: 'Paid invoices', value: counts.paid.toString(), style: 'text-emerald-700 bg-emerald-50', icon: 'paid', trend: trend(counts.paid, previousCounts.paid) },
    { title: 'Overdue invoices', value: counts.overdue.toString(), style: 'text-red-700 bg-red-50', icon: 'overdue', trend: trend(counts.overdue, previousCounts.overdue) },
    { title: 'Sent invoices', value: counts.sent.toString(), style: 'text-blue-700 bg-blue-50', icon: 'sent', trend: trend(counts.sent, previousCounts.sent) },
    { title: 'Draft invoices', value: counts.draft.toString(), style: 'text-slate-700 bg-slate-100', icon: 'draft', trend: trend(counts.draft, previousCounts.draft) },
  ];

  const revenueTrend = trend(
    currentInvoices.reduce((sum, invoice) => sum + Number(invoice.total), 0),
    previousInvoices.reduce((sum, invoice) => sum + Number(invoice.total), 0),
  );
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-wrap items-end justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.14em] text-blue-600">Overview</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">Revenue dashboard</h1>
          <p className="mt-1 text-sm text-slate-600">Track invoice health, revenue momentum, and payment performance.</p>
        </div>
        <div className="text-right">
          <p className="text-sm text-slate-500">Total revenue</p>
          <p className="text-2xl font-bold text-slate-900">{formatCurrency('USD', summary.totalRevenue)}</p>
          <p className={`text-xs font-semibold ${revenueTrend.positive ? 'text-emerald-600' : 'text-red-600'}`}>
            {revenueTrend.label} vs last month
          </p>
        </div>
      </section>
        ...
        {stats.map((card) => (
          ...
          >
            ...
            <p className={`mt-3 text-xs font-semibold ${card.trend.positive ? 'text-emerald-600' : 'text-red-600'}`}>
              {card.trend.label} vs last month
            </p>
          </article>
        ))}
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">Recent invoices</h2>
          <Link
        ...
        {summary.recentInvoices.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">Create your first invoice to start tracking revenue.</p>
            <Link
              href="/dashboard/invoices/new"
          ...
          <div className="space-y-2">
            {summary.recentInvoices.map((invoice) => (
              ...
              >
                ...
                <span className="text-sm font-semibold text-slate-900">{formatCurrency(invoice.currency, invoice.total)}</span>
              </Link>
            ))}
          </div>
        )}
